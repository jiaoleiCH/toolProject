...
let Data = {
    ...
    weChatKey : null,           // 微信后台保存分数的key
    worldRankData : null,       // 世界排行数据
}
Data.weChatKey  = `${Config.evn == EVN.debug ? 'test{}' : ''}${Data.proName}`

let MSG_MAP = {
    clienConfig     : "clienConfig",        // 客户端配置
    businessConfig  : "businessConfig",     // 商家配置
    adduser         : "adduser",            // 新用户
    addlogin        : "addlogin",           // 老用户
    login           : "login",              // 登录
    shareScheme     : "shareScheme",        // 通知服务器分享成功
    uploadIntegral1 : "uploadIntegral1",    // 兑换积分
    uploadRank      : "uploadRank",         // 上传分数到服务器
}
...
let MSG = {
    ...
    offMsgByKeyList(list) {
        if (!Array.isArray(list)) {
            console.error("args(list) is not an Array at offMsgByKeyList");
            return;
        }
...
let middleware = {
            ...
            if(!err){
                if (res.code == -1) {
                    ...
                    if (parseInt(res.data.kill)) {
                        ...
                        Data.isNewUser = res.data.new_user;
                        ...
                        Utils2.setLocalData("date", {time : new Date().toLocaleDateString(), counts : Data.convertCounts});
                        data_.cb ? data_.cb(res) : null;
    ...
    _adduser() {
        let data_ = [...arguments][0];
        Http.sendData(Config.requstUrl + "adduser", data_.data[0], (err,res) => {
            if(!err){
                if (res.code == -1) {
                    console.log("服务器异常")
                    WeChat.tipsFunc = () => {
                    ...
                    data_.cb ? data_.cb(res) : null;
                    console.log(`---------完成${MSG_MAP.adduser}消息回调---------`);
                }
            } else {
                WeChat.tipsFunc = () => {
                    if (WeChat.userInfoButton) {
    ...
    _addlogin() {
        let data_ = [...arguments][0];
        Http.sendData(Config.requstUrl + "addlogin", data_.data[0], (err,res) => {
            if(!err){
                if (res.code == -1) {
                    console.log("服务器异常")
                    WeChat.tipsFunc = () => {
                    ...
                    data_.cb ? data_.cb(res) : null;
                    console.log(`---------完成${MSG_MAP.addlogin}消息回调---------`);
                }
            } else {
                WeChat.tipsFunc = () => {
                    if (WeChat.userInfoButton) {
        ...
        Http.sendData(Config.requstUrl + "UploadRank", data_.data[0], (err,res) => {
            ...
            if(!err){
                if (res.code == -1) {
                    ...
                    // console.log("==================>>>>>", res);
                    Data.worldRankData = {list : res.list, my : res.curPlayer};
                    data_.cb ? data_.cb(res) : null;
